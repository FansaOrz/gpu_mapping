...
void update_piece_map(const std::string file_name, pcl::PointCloud<Point> current_pointcloud)
{
    // 若file已存在,更新; 否则,新建.
    std::mutex mu;
    std::unique_lock<std::mutex> q(mu);
    // current_pointcloud.points.resize(current_pointcloud.width * current_pointcloud.height);
    std::cout << "height = " << current_pointcloud.height << std::endl;
    std::cout << "width  = " << current_pointcloud.width << std::endl;
    std::cout << "height*width = " << current_pointcloud.height * current_pointcloud.width << std::endl;
    std::cout << "size = " << current_pointcloud.size() << std::endl;
    if (boost::filesystem::exists(file_name) == true)
    {
        std::cout << "file: " << file_name << " exists" << std::endl;
        pcl::PointCloud<Point>::Ptr pc(new pcl::PointCloud<Point>());
        pcl::io::loadPCDFile(file_name, *pc);
        *pc += current_pointcloud;
        pc->points.resize(pc->width * pc->height);
        pcl::io::savePCDFile(file_name, *pc);
    }
    else
    {
        pcl::io::savePCDFile(file_name, current_pointcloud);
    }
}
...
void LidarMapping::param_initial(ros::NodeHandle &nh, ros::NodeHandle &privateHandle)
{
    ...
    privateHandle.param<float>("passthrough_ground_height", passthrough_ground_height, -1.0);
    ...
    privateHandle.param<double>("voxel_leaf_size", voxel_leaf_size, 0.5);  //对输入的 一帧点云做第一次降采样
    privateHandle.param<double>("costmap_voxel_size", costmap_voxel_size, 0.1); //输出用于costmap的点云地图的降采样
    privateHandle.param<double>("global_voxel_leafsize", param_global_voxel_leafsize, 1.0);  //将一帧点云添加到 full_map之前降采样
    privateHandle.param<double>("min_scan_range", min_scan_range, 0.01);  ////提取一帧雷达中 自定义的区域
    ...
    std::cout << "passthrough_ground_height: " << passthrough_ground_height << std::endl;
    ...
    privateHandle.param<double>("piece_step", param_piece_step, 10.0);
...
bool LidarMapping::pose2GeometryPose__(geometry_msgs::Pose &to, const pose &from)
{
...
void LidarMapping::imu_calc(ros::Time current_time1)
{
    static ros::Time previous_time1 = current_time1;//局部变量的值在函数调用结束后不消失,而保留原值
    double diff_time = (current_time1 - previous_time1).toSec();
    ...
    current_velocity_imu_x += accX * diff_time; // imu的速度值会通过slam进行修正,以避免累计误差   line806
    ...
    guess_pose_imu.x = previous_pose.x + offset_imu_x; //previous_pose只有在完成一次ndt定位之后在更新previous_pose 
    guess_pose_imu.y = previous_pose.y + offset_imu_y;  //    大约更新40次guess_pose_imu 才执行一次previous_pose
...
void LidarMapping::imu_callback(const sensor_msgs::Imu::Ptr &input)
{
    ...
    const ros::Time current_time2 = input->header.stamp;
    static ros::Time previous_time2 = current_time2;
    const double diff_time = (current_time2 - previous_time2).toSec();
...
void LidarMapping::points_callback(const sensor_msgs::PointCloud2::ConstPtr &input)
{
    ...
    for (auto point : tmp.points)  //提取一帧雷达中 自定义的区域
    {
    ...
    if (initial_scan_loaded == 0) //如果是第一帧数据
    {
        pcl::transformPointCloud(*scan_ptr, *transformed_scan_ptr, tf_btol); // tf_btol为初始变换矩阵
        map += *transformed_scan_ptr;  // map   --用于ndt匹配的地图 
        global_map += *transformed_scan_ptr; //存储全局地图 也就是我们最终保存的地图
    ...
    ros::Time test_time_2 = ros::Time::now(); // TODO:  测试算法运行时间用
   ...
   //不借助imu或者里程计 预测NDT的初值,加速计算过程
    ...
    // 根据是否使用imu和odom,按照不同方式更新guess_pose(xyz,or/and rpy)  ??在这里计算有意义吗???
    ...
    if (_use_imu && !_use_odom)
    {
       //imu_calc(current_scan_time);
        ...
        pose2GeometryPose__(msg_imu_current_pose_.pose, guess_pose_imu);
        msg_imu_current_pose_.header.stamp=ros::Time::now();
        msg_imu_current_pose_.header.frame_id="/map";
    ...
    else if (_method_type == MethodType::use_gpu)
    {
            ...
            t_localizer = gpu_ndt.getFinalTransformation();  //变换矩阵!!!!
    ...
    if (final_num_iteration > max_iter)
    {
    ...
    // Update localizer_pose.  // 更新雷达坐标系 的坐标
    localizer_pose.x = t_localizer(0, 3);
    localizer_pose.y = t_localizer(1, 3);
    localizer_pose.z = t_localizer(2, 3);   //
    mat_l.getRPY(localizer_pose.roll, localizer_pose.pitch, localizer_pose.yaw, 1);

    // Update ndt_pose.  车坐标系 坐标
    ndt_pose.x = t_base_link(0, 3);
    ndt_pose.y = t_base_link(1, 3);
    ndt_pose.z = t_base_link(2, 3);
    mat_b.getRPY(ndt_pose.roll, ndt_pose.pitch, ndt_pose.yaw, 1);
    // current_pose   车坐标系 坐标
    ...
    br.sendTransform(tf::StampedTransform(transform, current_scan_time, "map", "base_link"));//发布 TF变换
    ...
    current_pose_imu.x = current_pose.x; //实时更新imu位置
    ...
    if (is_publish_map_for_costmap)
    {
        filter.setIfClipHeight(true);   //ground_filter.hpp   line 94
        ...
        pass.setInputCloud(local_no_ground);
        pass.setFilterFieldName("z");
        pass.setFilterLimits(passthrough_ground_height,10);
        pass.filter(*local_no_ground);
        ...
        for (size_t i = 0; i < local_no_ground->points.size(); i++)
        {
            ...
            if (dist > 6.0 && dist < 50.0)  //一帧点云里可以用的数据
                indices.indices.push_back(i);
        }
        ...
        voxel1.setLeafSize(costmap_voxel_size, costmap_voxel_size, costmap_voxel_size);
        ...
        // if (whole_cost_map.points.size() > 3000000)  //
        // {
        ...
        // }
        sensor_msgs::PointCloud2::Ptr costmap_cloud_ptr(new sensor_msgs::PointCloud2);
        pcl::toROSMsg(whole_cost_map, *costmap_cloud_ptr);
        costmap_cloud_pub.publish(*costmap_cloud_ptr);
    }
    

    // 按照步长,处理生成全局匹配用地图等
    //设置步长的目的是 减少叠加的激光 帧数 防止地图过于稠密
    double shift = sqrt(pow(current_pose.x - added_pose.x, 2.0) + pow(current_pose.y - added_pose.y, 2.0));
    if (shift >= min_add_scan_shift)  //更新 map  和  map_full   将新的一帧数据加上去
    {
        ...
        map += *to_map;  //更新 用于 ndt匹配的地图

        // 发布做下采样后的全局地图, 也就是我们用于重定位的点云地图
        if (is_publish_map_full)
        {
        ...
        {
            try_to_create_piece_map(current_pose, to_globalmap);
        }
    } // end if(shift)

    pcl::PointCloud<Point>::Ptr target_map(new pcl::PointCloud<Point>());
    if (shift >= param_min_update_target_map)  //更新 ndt的 InputTarget
    {
        target_map->points.clear();
        for (auto point : map.points)  //取规定半径内的点  只和局部的全局地图做匹配
        {
        ...
        else if (_method_type == MethodType::use_cpu) // 只是说map更新了,因此target也要更新,不要落后太多
        {
            ...
            #ifdef CUDA_FOUND
                    gpu_ndt.setInputTarget(target_map);   //将局部的全局地图做匹配作为下一时点云的匹配目标  target_map  
            #else
                    ROS_ERROR("set to ndt_gpu, but no cuda found!");
            #endif
        else if (_method_type == MethodType::use_omp)
            omp_ndt.setInputTarget(map_ptr);//之前的全部 点云 作为下一时点云的匹配目标    map_ptr
    }
    ...
    // end5
    path_msg.header.stamp=current_scan_time;
...
void LidarMapping::try_to_create_piece_map(const pose cur_pose, const pcl::PointCloud<Point>::ConstPtr &to_global_map)
{
    current_x_index = int(cur_pose.x / param_piece_step);
    current_y_index = int(cur_pose.y / param_piece_step);
    std::stringstream file_name;
    if (is_first_piece_map)
    {
        before_x_index = current_x_index;
        before_y_index = current_y_index;
        is_first_piece_map = false;
    }

    piece_map_container += *to_global_map;
    piece_map_container.points.resize(piece_map_container.width * piece_map_container.height);
    if (current_x_index != before_x_index || current_y_index != before_y_index)
    {
        file_name << param_piece_map_folder << "x" << before_x_index << "y" << before_y_index << ".pcd";
        std::thread t(update_piece_map, file_name.str(), piece_map_container);
        t.detach();
        before_x_index = current_x_index;
        before_y_index = current_y_index;
...
void LidarMapping::run(ros::NodeHandle &nh, ros::NodeHandle &private_nh)
{
    ...
    costmap_cloud_pub = private_nh.advertise<sensor_msgs::PointCloud2>("/global_map/costmap_cloud",1000);
    pub_imupose = private_nh.advertise<geometry_msgs::PoseStamped>("/imu_current_pose",10);
        pub_path = private_nh.advertise<nav_msgs::Path>("path",10, true);
    ...
    points_sub = private_nh.subscribe(_lidar_topic, 10, &LidarMapping::points_callback, this);
