...
#include<opencv2/core/core.hpp> //对输出的栅格地图做腐蚀操作
...
class MapGenerator
{
  ...
  public:
      ...
      : mapname_(mapname), saved_map_(false), threshold_occupied_(threshold_occupied), threshold_free_(threshold_free)
    {
      ...
      //map_sub_ = n.subscribe("map", 1, &MapGenerator::mapCallback, this); projected_map
    ...
    void mapCallback(const nav_msgs::OccupancyGridConstPtr& map)
    {
      ...
      //     //载入原图
      // Mat image = imread(mapdatafile);
      //  //获取自定义核
      // Mat element = getStructuringElement(MORPH_RECT, Size(5, 5));  //根据实际地图修改参数
      // Mat out1;
      //  //进行腐蚀操作
      // erode(image,out1, element);
      // imwrite("erode_map.pgm",out1);
      // std::cout<<"  Success generate erode map!!   "<<endl<<endl;
